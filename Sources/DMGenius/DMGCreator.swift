...
public enum TargetChoice: String, CaseIterable, Identifiable {
...
public struct DMGConfiguration {
    public var sourceURL: URL? {
        didSet { updateVolumeName() }
    }
    public var outputURL: URL?
    public var volumeName: String = "DMGenius Volume"
    public var targetChoice: TargetChoice = .applications
    public var customTargetPath: String? = nil // path relative to root of dmg, e.g. "/Users/alice/Applications"
    public var includeApplicationsSymlink: Bool = true

    public init() { updateVolumeName() }
...
public struct RealProcessRunner: ProcessRunning {
    public init() {}
    public func run(_ launchPath: String, _ arguments: [String]) throws -> (output: String, error: String) {
        let proc = Process()
...
public class DMGCreator: ObservableObject {
    public static let shared = DMGCreator()

    private let fileManager = FileManager.default
    private let processRunner: ProcessRunning

    @Published public var isWorking: Bool = false
    @Published public var progressMessage: String? = nil

    public init(processRunner: ProcessRunning = RealProcessRunner()) {
    ...
    public func createDMG(with config: DMGConfiguration, completion: @escaping (Result<URL, Error>)->Void) {
    ...
    public func createStructureOnlyDMG(with config: DMGConfiguration, completion: @escaping (Result<URL, Error>)->Void) {
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            guard let self = self else {
                completion(.failure(NSError(domain: "DMGCreator", code: 11, userInfo: [NSLocalizedDescriptionKey: "DMGCreator deallocated"])))
                return
    ...
    public func buildTargetStructure(in stagingRoot: URL, config: DMGConfiguration) throws -> URL {
    ...
    public func copySourceToStaging(source: URL, dest: URL) throws {
    ...
    public func setPermissions(for url: URL, permissions: UInt16) throws {
    ...
    public func createApplicationsSymlink(in stagingRoot: URL) throws {
    ...
    public func createCompressedReadOnlyDMG(stagingRoot: URL, outputURL: URL, volumeName: String) throws {
