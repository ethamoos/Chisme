...
final class DMGItem: Identifiable, ObservableObject {
    let id = UUID()
    let url: URL
    let name: String
...
@MainActor
final class DMGManager: ObservableObject {
    @Published var folderPath: URL?
    @Published private(set) var items: [DMGItem] = []
    ...
    func runScriptOnMounted(script: String? = nil, delaySeconds: Int, requireAdmin: Bool) async -> Bool {
    ...
    func unmount(item: DMGItem) async {
    ...
    func runSequential(relativePath: String, delaySeconds: Int, requireAdmin: Bool) async {
